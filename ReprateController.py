...
class ReprateDS(Device):
    ...
    event1_frequency_r1 = attribute(label='R1 event1 frequency',
                                    dtype=float,
                                    access=pt.AttrWriteType.READ,
                                    unit="Hz",
                                    format="%2.1f",
                                    ...
                                    max_value=100.0,
                                    fisallowed="",
                                    fget="get_event1_frequency_r1",
                                    doc="Frequency for event 1",
                                    )

    event1_frequency_r3 = attribute(label='R3 event1 frequency',
                                    dtype=float,
                                    access=pt.AttrWriteType.READ,
                                    unit="Hz",
                                    format="%2.1f",
                                    ...
                                    max_value=100.0,
                                    fisallowed="",
                                    fget="get_event1_frequency_r3",
                                    doc="Frequency for event 1",
                                    )

    current_ring_target = attribute(label='current ring target',
                                    dtype=str,
                                    access=pt.AttrWriteType.READ_WRITE,
                                    unit="",
                                    format="%s",
                                    fget="get_current_ring",
                                    fset="set_current_ring",
                                    memorized=True,
                                    hw_memorized=True,
                                    doc="Ring selected to trigger chopper", )
    ...
    kicker_evr_name_r1 = device_property(dtype=str,
                                         doc="Full Tango name for the EVR attribute triggering the kicker magnet for R1,"
                                             "e.g. R1-D110210CAB04/TIM/EVR-01/Output00Delay",
                                         default_value="R1-D110210CAB04/TIM/EVR-01/Output00Delay")

    chopper_evr_name_r1 = device_property(dtype=str,
                                          doc="Full Tango name for the EVR attribute triggering the chopper for R1,"
    ...
    kicker_evr_name_r3 = device_property(dtype=str,
                                         doc="Full Tango name for the EVR attribute triggering the kicker magnet for R3,"
                                             "e.g. R3-A110111CAB04/TIM/EVR-01/Output00Delay",
                                         default_value="R3-A110111CAB04/TIM/EVR-01/Output00Delay")

    chopper_evr_name_r3 = device_property(dtype=str,
    ...
    def __init__(self, klass, name):
        ...
        self.kicker_evr_device_r1 = None
        self.chopper_evr_device_r1 = None
        self.evg_device_r3 = None
        self.kicker_evr_device_r3 = None
        self.chopper_evr_device_r3 = None
        self.event_time_offset_r1_data = None
        self.event_time_offset_r3_data = None
        self.standard_table_r1_data = None
        self.standard_table_r3_data = None
        self.dual_reprate_table_r1_data = None
        self.dual_reprate_table_r3_data = None
    ...
    def init_device(self):
        ...
        try:
            dev_name = "/".join(self.kicker_evr_name_r1.split("/")[0:3])
            self.kicker_evr_device_r1 = pt.DeviceProxy(dev_name)
        except pt.DevFailed as e:
            self.error_stream("Could not connect to device {0}. {1}".format(self.kicker_evr_name_r1, e))
            self.kicker_evr_device_r1 = None
            self.set_state(pt.DevState.UNKNOWN)
            self.set_status("Could not connect to {0}: {1}".format(dev_name, e))
            return
        self.info_stream("Connected to EVR device {0}".format(self.kicker_evr_name_r1))
        try:
            dev_name = "/".join(self.chopper_evr_name_r1.split("/")[0:3])
            self.chopper_evr_device_r1 = pt.DeviceProxy(dev_name)
        except pt.DevFailed as e:
            self.error_stream("Could not connect to device {0}. {1}".format(self.chopper_evr_name_r1, e))
            self.chopper_evr_device_r1 = None
            self.set_state(pt.DevState.UNKNOWN)
            self.set_status("Could not connect to {0}: {1}".format(dev_name, e))
            return
        self.info_stream("Connected to EVR device {0}".format(self.chopper_evr_name_r1))
        ...
        try:
            dev_name = "/".join(self.kicker_evr_name_r3.split("/")[0:3])
            self.kicker_evr_device_r3 = pt.DeviceProxy(dev_name)
        except pt.DevFailed as e:
            self.error_stream("Could not connect to device {0}. {1}".format(self.kicker_evr_name_r3, e))
            self.kicker_evr_device_r3 = None
            self.set_state(pt.DevState.UNKNOWN)
            self.set_status("Could not connect to {0}: {1}".format(dev_name, e))
            return
        self.info_stream("Connected to EVR device {0}".format(self.kicker_evr_name_r3))
        try:
            dev_name = "/".join(self.chopper_evr_name_r3.split("/")[0:3])
            self.chopper_evr_device_r3 = pt.DeviceProxy(dev_name)
        except pt.DevFailed as e:
            self.error_stream("Could not connect to device {0}. {1}".format(self.chopper_evr_name_r3, e))
            self.chopper_evr_device_r3 = None
            self.set_state(pt.DevState.UNKNOWN)
            self.set_status("Could not connect to {0}: {1}".format(dev_name, e))
            return
        self.info_stream("Connected to EVR device {0}".format(self.chopper_evr_name_r3))
        ...
        # Define the initial configuration as "STANDARD", whatever it is.
        self.status_r1_table = "STANDARD"
        self.status_r3_table = "STANDARD"
        self._gen_status()
    ...
    def _gen_status(self):
        ...
        """
        st = self.get_state()
        if st in [pt.DevState.RUNNING, pt.DevState.ON]:
            s = "R1 table: {0}\n" \
                "R3 table: {1}\n" \
                "Target: {2}\n\n" \
                "{3}".format(self.status_r1_table.upper(), self.status_r3_table.upper(),
                             self.current_ring_data.upper(), st)
            self.set_status(s)
        return True
    ...
    def read_sequence_from_device(self):
        ...
        if self.evg_device_r1 is not None:
            ...
            try:
                ...
                with self.data_lock:
                    self.standard_table_r1_data = inj_table
        ...
        if ring == "r1":
            if self.evg_device_r1 is not None:
                try:
                    ...
                    with self.data_lock:
                        self.injection_table_r1_data = inj_table
                        self.event1_frequency_r1_data = 1.0 / t_p
            ...
            if self.evg_device_r3 is not None:
                try:
                    ...
                    with self.data_lock:
                        ...
                        self.event1_frequency_r3_data = 1.0 / t_p
    ...
    def generate_sequence_factor(self, f0, factor, event0, event1):
        ...
        t0 = np.arange(factor) / f0 / t_mrf + 1         # Minimum timestamp value = 1
        t1 = np.arange(1) / f1 / t_mrf + 2              # Next timestamp must be higher than the previous to activate
    ...
    def configure_evr_outputevent(self, dev, output, event, do_init=True):
        ...
        :param dev: Tango DeviceProxy
        :param output: EVR output #, 0-11
        :param event: Event code, 0x--
        :return:
        """
        output_list = list(dev.get_property("Output_Init")["Output_Init"])
        if isinstance(output, str):
            s = output
            output = int(s.split("Output")[-1].split("Delay")[0])
    ...
    def write_sequence_to_device(self, inj_table, ring, evr_event=0xa4):
        ...
        :type ring: str
        :param evr_event: Event to set for the event receivers. 0xa4 is for dual reprate, 0xa0 is single reprate
        :return:
        :rtype:
        """
        ...
        timestamp_t = [it[0] for it in inj_table]
        event_t = [hex(it[1]) for it in inj_table]
        enable_t = [it[2] for it in inj_table]

        timestamp = [max(timestamp_t)] * self.n_max
        event = ["0x7f"] * self.n_max
        enable = [1] * self.n_max

        self.info_stream("\nTimestamps: {0}\nEvents: {1}\nEnable: {2}".format(timestamp, event, enable))

        attr_len = len(timestamp_t)
        if attr_len > self.n_max:
            attr_len = self.n_max
        for k in range(attr_len):
            timestamp[k] = timestamp_t[k]
            event[k] = event_t[k]
            enable[k] = enable_t[k]
        ...
        try:
            ...
            self.set_state(pt.DevState.UNKNOWN)
            self.set_status("Error {1} init. {0}".format(e, ring))
            self.error_stream("Error {1} init. {0}".format(e, ring))
            raise e
        self.info_stream("{0} Generator init".format(ring))

        self.read_inj_table_from_device(ring)

        # Start injection manually after
    ...
    def get_standard_table_r1(self):
        with self.data_lock:
            inj_table = copy.copy(self.standard_table_r1_data)
        return str(inj_table)
    ...
    def get_standard_table_r3(self):
        with self.data_lock:
            inj_table = copy.copy(self.standard_table_r3_data)
        return str(inj_table)
    ...
    def is_event0_frequency_r1_allowed(self):
        self.debug_stream("is_event0_frequency_r1_allowed")
        if self.get_state() in [pt.DevState.ON, pt.DevState.RUNNING]:
            return True
        else:
            return False
    ...
    def get_event1_frequency_r1(self):
        with self.data_lock:
            f = self.event1_frequency_r1_data
        return f

    def is_event1_frequency_r1_allowed(self):
        if self.get_state() in [pt.DevState.ON, pt.DevState.RUNNING]:
            return True
        else:
            return False

    def get_event1_frequency_r3(self):
        with self.data_lock:
            f = self.event1_frequency_r3_data
        return f

    def is_event1_frequency_r3_allowed(self):
        if self.get_state() in [pt.DevState.ON, pt.DevState.RUNNING]:
            return True
        else:
            return False
    ...
    def set_current_ring(self, ring):
        self.info_stream("Setting ring target to {0}".format(ring))
        with self.data_lock:
            try:
    ...
    def set_dual_reprate_table_to_r1(self):
        if self.get_state() in [pt.DevState.ON, pt.DevState.RUNNING]:
            inj_table = self.dual_reprate_table_r1_data
            self.write_sequence_to_device(inj_table, "r1", self.event1_code)
            self.set_state(pt.DevState.RUNNING)
    ...
    def set_dual_reprate_table_to_r3(self):
        if self.get_state() in [pt.DevState.ON, pt.DevState.RUNNING]:
            inj_table = self.dual_reprate_table_r3_data
            self.write_sequence_to_device(inj_table, "r3", self.event1_code)
            self.set_state(pt.DevState.RUNNING)
    ...
    def set_current_std_table_to_r1(self):
        if self.get_state() in [pt.DevState.ON, pt.DevState.RUNNING]:
            inj_table = self.standard_table_r1_data
            self.write_sequence_to_device(inj_table, "r1", self.event0_code)
            self.set_state(pt.DevState.RUNNING)
            self.status_r1_table = "STANDARD"
            self._gen_status()
    ...
    def set_current_std_table_to_r3(self):
        if self.get_state() in [pt.DevState.ON, pt.DevState.RUNNING]:
            inj_table = self.standard_table_r3_data
            self.write_sequence_to_device(inj_table, "r3", self.event0_code)
            self.set_state(pt.DevState.RUNNING)
            self.status_r3_table = "STANDARD"
            self._gen_status()
    ...
    def store_current_std_table_to_ref_r1(self):
        s = str(self.standard_table_r1_data)
    ...
    def store_current_std_table_to_ref_r3(self):
        s = str(self.standard_table_r3_data)
    ...
    def set_ref_table_to_r1(self):
        if self.get_state() in [pt.DevState.ON, pt.DevState.RUNNING]:
            self.write_sequence_to_device(json.loads(self.ref_std_table_r1), "r1", self.event0_code)
    ...
    def set_ref_table_to_r3(self):
        if self.get_state() in [pt.DevState.ON, pt.DevState.RUNNING]:
            self.write_sequence_to_device(json.loads(self.ref_std_table_r3), "r3", self.event0_code)
